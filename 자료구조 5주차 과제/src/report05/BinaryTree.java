...
public class BinaryTree<Key extends Comparable <Key>> {
	...
	public void postorder(BTNode<Integer>n) {  //후위 순회
    	if(n!= null) {
    		...
    		postorder(n.getRight()); //n의 오른쪽 ...
    		System.out.print(n.getKey() +" "); //노드 n 방문
    	}
    }
	
	public int size(BTNode<Integer>n) { //n을 루트로하는 서브트리에 있는 노드 수
    	if(n==null)
    		return 0;
    	else
	...
	public int height(BTNode<Integer>n) { //n을 루트로하는 서브트리의 높이
    	if(n==null)
    		return 0;
    	else
	...
	public void levelorder(BTNode<Integer>root) { //레벨 순회
    	...
    	while(!q.isEmpty()) {
    		...
    		if(t.getLeft() != null)//제거된 왼쪽지ㅏ식이 null이 아니면
    			q.add(t.getLeft()); //큐에 왼쪽 자식 삽입
    		if(t.getRight() != null) //제거된 오른족 자식이 ..
    			q.add(t.getRight()); //큐에 오른쪽 잣기 삽입
    	}
    }
}